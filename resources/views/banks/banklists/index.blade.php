    ...
    <div class="jrr-class">
        ...
        <div class="box box-danger">
            ...
            <div class="box-body">
                ...
                @include('banks.banklists.create')
                @include('banks.banklists.edit')
            </div>
        ...
        (function ($) {
            ...
            jQuery(document).ready(function ($) {
                ...
                $(document).on('click', '.edit_data',function() {
                    var banklist_id = $(this).attr('id');
                    var _token = $('input[name="_token"]').val();
                    $.ajax({
                        url: '{{ url("banks/banklistedit") }}',
                        method: "POST",
                        data: {banklist_id:banklist_id, _token : _token},
                        dataType:"json",
                        success:function(data){
                            $("#name").val(data.name);
                            $("#status").val(data.status);
                            $("#bank_hidden_id").val(data.id);
                        }
                    });
                });
                ...
                $(document).ready(function() {
                    $('#bank_table').DataTable({
                        "processing": true,
                        "serverSide": true,
                        "ajax": "{{ route('ajaxdata.getBanklistData') }}",
                        "columns":[
                            { "data": "serial" },
                            { "data": "name" },
                            {
